...
public class InteractionManager : MonoBehaviour
{
    ...
    private void Update()
    {
        if (!forceStay)
        {
            ...
            if (Physics.Raycast(transform.position, transform.forward, out RaycastHit hit, distance) && (interactable = hit.collider.GetComponentInChildren<Interactable>()) != null)
            {
        ...
        if (currentInteractable != null && Input.GetKey(Keybinds.Interact) && currentInteractable.IsEnabled)
        {
            if (currentTime == 0)
    ...
    private void ShowGUI(ref RaycastHit hit, ref Interactable i)
    {
        ...
        PlayerGuiManager.Instance.DisplayInteract(true, GuiText, Keybinds.Interact);
    }
